...
class ClassScheduleEventUpdate(
    LoginRequiredMixin,
    UserPassesTestMixin,
    UpdateView
):
    template_name = "drop_calendar/class_calendar_update.html"
    model = ClassScheduleEvent
    ...
    def get_context_data(self, **kwargs):
        kwargs = super(ClassScheduleEventUpdate, self).get_context_data()
        query = self.model.objects.custom_filter()
        event_arr = []
        if query:
            for i in query:
...
@login_required
def class_schedule_event_delete_view(request):
    try:
        ClassScheduleEvent.objects.filter(
            pk=request.GET.get('pk')
        ).delete()
    except Exception as ex:
        messages.warning(request, "Unable to Delete")
